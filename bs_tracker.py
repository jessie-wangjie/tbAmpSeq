...
def invoke_pipeline(sample_list: dict, run_info_json: dict):
    benchling = Benchling(url=api_url, auth_method=ApiKeyAuth(api_key))
    for s, id in sample_list.items():
        ...
        pd.Series(run_info_json).to_json(os.path.join(pipeline_run_name, "run.json"))
        subprocess.call("python /home/ubuntu/bin/tbOnT/tbAmpSeq.B2B.coordinates.py -m %s -i %s -p 8 -o %s" % (
            s, s, pipeline_run_name), shell=True)
        run_info_json = pd.read_json(os.path.join(pipeline_run_name, "run.json"), typ="series")
        ...
        entity = CustomEntityCreate(schema_id=schema_id, folder_id=folder_id, registry_id=registry_id,
                                    ...
                                    fields=fields({"Genomics AmpSeq Project Queue": {"value": s},
                                                   ...
                                                   "ELN entry": {"value": run_info_json["project_name"]},
                                                   "AmpSeq Project Name": {"value": ngs_name.id},
                                                   "run start": {"value": run_info_json["run start"]},
                                                   "run end": {"value": run_info_json["run end"]},
        ...
        subprocess.call(
            "aws s3 --profile=jwang sync %s s3://tb-ngs-analysis/%s --quiet" % (pipeline_run_name, pipeline_run_name),
            shell=True)
    ...
    while True:
        ...
        for run in response.json().get("Items"):
            ...
            if run["Status"] != "Complete" and run["Status"] != "Failed":
                ...
                invoke_pipeline(samples, run_json)
